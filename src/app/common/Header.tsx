...
const LinkDropdown = (props: { tab: Tab }) => {
  ...
  return (
    <div
      className="flex absolute z-50 min-w-[10rem] p-2 justify-items-center flex-col bg-slate-700
                    transition-all ease-linear duration-300 rounded-md text-left"
    >
      {props.tab.leaves.map((leaf: Tab, index: number) => (
        <Link className="text-white opacity-70 m-1 hover:opacity-100 hover:underline" href={leaf.route} key={index}>
          {leaf.text}
        </Link>
      ))}
    </div>
...
export const Header = () => (
  <>
    <header className="flex w-full pt-2 pb-2 pr-4 pl-4">
      <nav
        className="flex flex-row place-self-center items-center
      ...
      >
        ...
        <div className="flex flex-row text-sm w-full justify-around flex-wrap list-none items-center">
          {Tabs.filter(tab => tab.text !== "Home").map((tab: Tab, index: number) =>
            tab.leaves ? (
              <ExpandableLinkEntry tab={tab} key={index} />
            ) : (
              <Link className="text-white opacity-60 hover:underline hover:opacity-100" href={tab.route} key={index}>
                {tab.text} {tab.leaves ? <FontAwesomeIcon icon={faCaretDown} /> : ""}
              </Link>
            )
          )}
